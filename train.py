...
def load_data():
    ...
    with open(data_file) as fp:
        for i, line in enumerate(fp):
            ...
            click_data =np.array(list(map(np.int64, info[3:803])))
            demo_data = np.array(list(map(np.int64, info[803:839])))
            # print(info[-2])
            # print(info[-1].strip())
            if args.num_classes==2:
                y = list(map(np.int64, info[-2]))
            else:
                y = list(map(np.int64, info[-1].strip()))
            ...
            # print(nodes.index(student_id))
            # print(nodes.index(course_id))
            # print(y[0])
...
def run_simulation():
    ...
    # print(train_edges[:,2])
    # print(train_edge_type)
    train_edge_norm =np.ones(len(train_edge_dst), dtype=np.int64) #/ degrees.astype(np.float32)
    ...
    # train_edge_src=torch.as_tensor(train_edge_src, dtype=torch.int64)
    # train_edge_dst=torch.as_tensor(train_edge_dst, dtype=torch.int64)
    print(train_edge_src)
